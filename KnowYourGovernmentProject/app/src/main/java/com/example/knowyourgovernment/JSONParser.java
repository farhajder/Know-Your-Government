...
public class JSONParser {
    ...
    private List<CivilGovernmentOfficial> civilGovernmentOfficialList;
    ...
    public List<CivilGovernmentOfficial> readCivilData(InputStream in) throws IOException {
        JsonReader jsonReader = new JsonReader(new InputStreamReader(in, "UTF-8"));
        try { readCivilOfficialObject(jsonReader); }
        finally { jsonReader.close(); }
        in.reset();
        jsonReader = new JsonReader(new InputStreamReader(in, "UTF-8"));
        try { civilGovernmentOfficialList = readCivilOfficeObject(jsonReader); }
        finally { jsonReader.close(); }
        return civilGovernmentOfficialList;
    }

    private List<CivilGovernmentOfficial> readCivilOfficeObject(JsonReader reader) throws IOException {
        civilGovernmentOfficialList = new ArrayList<>();
        reader.beginObject();
    ...
    private void readCivilOfficeObjectList(JsonReader reader) throws IOException {
        ...
        while (reader.hasNext()) {
            String name = reader.nextName();
            if (name.equals("name")) { officeName = reader.nextString(); }
            else if (name.equals("officialIndices")) {
                reader.beginArray();
                while (reader.hasNext()) {
                    int index = reader.nextInt();
                    CivilOfficial civilOfficial = civilofficialOfficeMap.get(index);
                    civilGovernmentOfficialList.add(new CivilGovernmentOfficial(officeName,index, civilOfficial));
    ...
    private CivilOfficial readCivilOfficialsObjectList(JsonReader reader) throws IOException {
        ...
        while (reader.hasNext()) {
            String name = reader.nextName();
            if (name.equals("name")) { officialName = reader.nextString(); }
            else if (name.equals("address")) { officialAddress =  readCivilOfficialAddressArray(reader); }
            else if (name.equals("party")) { partyName=reader.nextString(); }
            else if (name.equals("phones")) { phNumList =  readContactDetails(reader); }
            else if (name.equals("emails")) { emailList=  readEmailDetails(reader); }
            else if (name.equals("urls")) { urlList =  readWebsiteDetails(reader); }
            else if (name.equals("photoUrl")) { photoUrl = reader.nextString(); }
    ...
    private OfficialAddress readCivilOfficialAddressArray(JsonReader reader) throws IOException {
        ...
        while (reader.hasNext()) {
            ...
            while (reader.hasNext()) {
                ...
                if (nextName.equals("line1")) { line1 = reader.nextString(); }
                else if (nextName.equals("line2")) { line2 = reader.nextString(); }
                else if (nextName.equals("line3")) { line3 = reader.nextString(); }
                else if (nextName.equals("city")) { city = reader.nextString(); }
                else if (nextName.equals("state")) { state = reader.nextString(); }
                else if (nextName.equals("zip")) { zip = reader.nextString(); }
                else { reader.skipValue(); }
            }
            reader.endObject();
    ...
    private void readSocialMediaDetails(JsonReader reader, SocialMediaChannel socialMediaChannel)throws IOException {
        ...
        while (reader.hasNext()) {
            ...
            while (reader.hasNext()){
                String name = reader.nextName();
                if (name.equals("type")) { channelName = reader.nextString(); }
                else if (name.equals("id")) { channelID = reader.nextString(); }
                else{ reader.skipValue(); }
            }
            if (channelName.equalsIgnoreCase("GooglePlus")){ socialMediaChannel.setGooglePageID(channelID); }
            else if(channelName.equalsIgnoreCase("Facebook")){ socialMediaChannel.setFacebookPageID(channelID); }
            else if(channelName.equalsIgnoreCase("Twitter")){ socialMediaChannel.setTwitterPageID(channelID); }
            else if(channelName.equalsIgnoreCase("YouTube")){ socialMediaChannel.setYoutubePageID(channelID); }
            reader.endObject();
        }
        reader.endArray();
    }
    ...
    public OfficialAddress readAddressDetails(InputStream in) throws IOException {
        OfficialAddress officialAddress =null;
        JsonReader jsonReader = new JsonReader(new InputStreamReader(in, "UTF-8"));
        try { officialAddress =   readAddressObject(jsonReader); }
        finally { jsonReader.close(); }
        return officialAddress;
    }
